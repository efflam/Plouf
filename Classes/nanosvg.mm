...
static void svgBeginShape(struct SVGParser* p)
{
	p->npbuf = 0;
	p->nvbuf = 0;
}
...
static void svgEndShape(struct SVGParser* p)
{
	float* t;
	float* pt;
	...
	for (i = 0; i < p->npbuf; ++i)
	{
		float* pts = &p->vbuf[p->pbuf[i*2+0]*2];
		int npts = p->pbuf[i*2+1];
		char closed = 0;
		if (npts < 0)
		{
			closed = 1;
...
static void svgBeginPath(struct SVGParser* p)
{
	if (p->npbuf+1 > p->cpbuf)
	{
		int cap = p->cpbuf ? p->cpbuf*2 : 8;
		int* buf = (int*)malloc(cap*2*sizeof(int));
		if (!buf) return;
		if (p->npbuf)
			memcpy(buf, p->pbuf, p->npbuf*2*sizeof(int));
		if (p->pbuf)
			free(p->pbuf);
		p->pbuf = buf;
		p->cpbuf = cap;
	...
	p->pbuf[p->npbuf*2+0] = p->nvbuf;
	p->pbuf[p->npbuf*2+1] = 0;
	p->npbuf++;
}

static void svgEndPath(struct SVGParser* p, char closed)
{
	if (p->npbuf > 0)
	{
		int start = p->pbuf[(p->npbuf-1)*2+0];
		int n = p->nvbuf - start;
		if (closed) n = -n;
		p->pbuf[(p->npbuf-1)*2+1] = n;
	}
}

static void svgPathPoint(struct SVGParser* p, float x, float y)
{
	if (p->nvbuf+1 > p->cvbuf)
	{
		int cap = p->cvbuf ? p->cvbuf*2 : 8;
		float* buf = (float*)malloc(cap*2*sizeof(float));
		if (!buf) return;
		if (p->nvbuf)
			memcpy(buf, p->vbuf, p->nvbuf*2*sizeof(float));
		if (p->vbuf)
			free(p->vbuf);
		p->vbuf = buf;
		p->cvbuf = cap;
	}
	p->vbuf[p->nvbuf*2+0] = x;
	p->vbuf[p->nvbuf*2+1] = y;
	p->nvbuf++;
}
